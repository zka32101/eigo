...
final userRegisteredAtOverrideProvider =
  ...
  final registeredAtString =
      prefs.getString('eigo_user_registered_at');
  ...
  await prefs.setString('eigo_user_registered_at', now.toIso8601String());
  return now;
});

/// eigo 用：サブスク購読状態プロバイダー
...
/// クイズへのアクセス可否（簡略版）
final canAccessEigoQuizzesProvider = FutureProvider<bool>((ref) async {
  final registeredAt =
      await ref.watch(userRegisteredAtOverrideProvider.future);
  final isSubscribed =
      await ref.watch(subscriptionStatusOverrideProvider.future);
...
/// ペイウォール表示が必要か
final shouldShowEigoPaywallProvider =
    FutureProvider<bool>((ref) async {
  final canAccess =
      await ref.watch(canAccessEigoQuizzesProvider.future);
  return !canAccess;
});

/// 無料期間の残り日数
final remainingEigoFreeDaysProvider =
